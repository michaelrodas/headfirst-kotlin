...
fun main(args: Array<String>) {
    ...
    val sumOfInts = ints.fold(0) { runningSum, item -> runningSum + item } //Sums all values in the list
    println("sumOfInts: $sumOfInts")

    val productOfInts = ints.fold(1) { runningProduct, item -> runningProduct * item } //Multiplies all values in the list
    println("productOfInts: $productOfInts")

    val names = groceries.fold("") { string, item -> string + " ${item.name}" }
    println("names: $names")

    val changeFrom50 = groceries.fold(50.0) { change, item //Subtracts all the elements on the list to the initial budget
        -> change - item.unitPrice * item.quantity }
    println("changeFrom50: $changeFrom50")

    println("Print the total cost of each category")
